        ...
        while (true) {
            ...
            for (Tile tile : myTiles) {
                ...
                if (tile.canBuild) {
                    ...
                    // TODO: pick whether to build recycler here
                    shouldBuild = tile.scrapAmount > 0;
                    if (shouldBuild) {
            ...
            for (Tile tile : myUnits) {
                Tile target = null; 
                // set target to the tile to move to
                target = neutralTiles.stream().min(Comparator.comparingInt(t -> t.scrapAmount)).orElse(null);
                if (target != null) {
                    int amount = 0; 
                    // TODO: pick amount of units to move
                    amount = Math.min(tile.units, target.units + 1);
